...
public class Student {
	...
	// TODO den walking-studenten hiervon erben lassen bzw irgendwie magically dafür
	// sorgen dass das walken woanders steht
	...
	Ausgabe current;
	int waitTicks;
	public int spentTicks; // DATA
	int additionallywants = -1; // entweder -1, dann will er nur ne Hauptmahlzeit, oder consts.SALAD oder
								// consts.POMMES
	...
	// wenn er gegen wände läuft läuft er in eine zufällige richtung. damit die
	// nicht jigglet muss er sie speichern.
	...
	private void global_construct(int num, SharedStuff sharedstuff, int fp, Context<Object> context) {
		...
		additionallywants_orig = additionallywants; // zum printen und für chooseMeal
		consideredBarsList = setConsideredBars(additionallywants); // wenn er Salat/Pommes ZUSÄTZLICH will will er das
																	// nicht mehr als Hauptmahlzeit.

		switch (fp) {
			case consts.MEAT:
				sharedstuff.studierendeMeatEater.add(this);   // DATA
				break;
			case consts.VEGGIE:
				sharedstuff.studierendeVeggie.add(this);
				break;
			case consts.VEGANER:
				sharedstuff.studierendeVegan.add(this);
				break;
			case consts.NOPREFERENCE:
				sharedstuff.studierendeNoPref.add(this);
				break;
		}
	}
	...
	public Student(int num, SharedStuff sharedstuff, int fp, Context<Object> context, Grid<Object> g, int x, int y) {
		...
		scheduledSteps.add(sharedstuff.schedule.schedule(
				ScheduleParameters.createRepeating(sharedstuff.schedule.getTickCount() + 1, 1), this, "step_grid"));
	}
	...
	protected String get_pref_string() {
		...
		case consts.MEAT:
			foodprefstring = "Meateater";
			break;
		case consts.VEGGIE:
			foodprefstring = "Vegetarier";
			break;
		case consts.VEGANER:
			foodprefstring = "Veganer";
			break;
		default:
			foodprefstring = "NoPrefEater";
			break;
		}
		if (additionallywants_orig == consts.ESSEN_SALAD)
			foodprefstring += "+Salad";
		if (additionallywants_orig == consts.ESSEN_POMMES)
			foodprefstring += "+Pommes";
		return foodprefstring;
	}

	// Überschrieben im StudentGoalOriented: der mags in jedem Fall da er eh nur
	// hingeht wo er's mag
	public boolean chooseMeal(Ausgabe currentBar) {
		...
		// Essen funktioniert so: wenn das Essen vom vegetarismus-Grad zu ihnen passt,
		// nehmen sie es zu einem gewissem Prozentsatz sofort. Wenn sie am Ende
		// alle Ausgaben abgelaufen sind, ohne was zu nehmen, nehmen sie das ein
		// zufälliges von denen die "am besten" zu ihnen passen.
		int essen = currentBar.getEssen();

		if (essen == additionallywants_orig) // wenn er zusätzlich salat/pommes will nimmt ers in jedem fall
												// (anderenfalls gehet er nicht an diese schlnage)
			return true;

		if (essen >= food_preference) { // foodpreference and food are sorted by priority: a person of type meat will
										// like the lowest one most. A person of type vegan will like the lowest one >=
										// itself (=2) most.
			if ((best_food_so_war_was > essen) && (food_preference != consts.NOPREFERENCE)) { // die mit no preference
																								// fügen einfach alles
																								// zu ihrer best_so_far
																								// liste hinzu, alle
																								// anderen nur die die
																								// sie am meisten
																								// mochten.
		...
		if (food_preference == consts.VEGGIE && consts.vegetarian.contains(essen)) { // VEGGIE
			if (essen == consts.ESSEN_VEGGIE && randomNum <= sharedstuff.foodParam[0])
				return true;
			if (essen == consts.ESSEN_VEGAN && randomNum <= sharedstuff.foodParam[1])
				return true;
			if (essen == consts.ESSEN_SALAD && randomNum <= sharedstuff.foodParam[2])
				return true;
			if (essen == consts.ESSEN_POMMES && randomNum <= sharedstuff.foodParam[3])
				return true;
		} else if (food_preference == consts.VEGANER && consts.vegan.contains(essen)) { // VEGAN
			if (essen == consts.ESSEN_VEGAN && randomNum <= sharedstuff.foodParam[4])
				return true;
			if (essen == consts.ESSEN_SALAD && randomNum <= sharedstuff.foodParam[5])
				return true;
			if (essen == consts.ESSEN_POMMES && randomNum <= sharedstuff.foodParam[6])
				return true;
		} else if (food_preference == consts.MEAT && consts.meatlover.contains(essen)) { // MEAT
			if (essen == consts.ESSEN_VEGGIE && randomNum <= sharedstuff.foodParam[7])
				return true;
			if (essen == consts.ESSEN_VEGAN && randomNum <= sharedstuff.foodParam[8])
				return true;
			if (essen == consts.ESSEN_MEAT && randomNum <= sharedstuff.foodParam[9])
				return true;
			if (essen == consts.ESSEN_SALAD && randomNum <= sharedstuff.foodParam[10])
				return true;
			if (essen == consts.ESSEN_POMMES && randomNum <= sharedstuff.foodParam[11])
				return true;
	...
	public void create_food_obj(Ausgabe currentBar) {
		int essen = currentBar.getEssen();
		if (essen == consts.ESSEN_VEGGIE)
			new VeggieObj(sharedstuff.foodContext);
		if (essen == consts.ESSEN_VEGAN)
			new VeganObj(sharedstuff.foodContext);
		if (essen == consts.ESSEN_MEAT)
			new MeatObj(sharedstuff.foodContext);
		if (essen == consts.ESSEN_SALAD)
			new SaladObj(sharedstuff.foodContext);
		if (essen == consts.ESSEN_POMMES)
			new PommesObj(sharedstuff.foodContext);
	}
	...
	public void remove_me() {
		...
		sharedstuff.schedule.schedule(ScheduleParameters.createOneTime(sharedstuff.schedule.getTickCount() + 0.01, 1),
				sharedstuff.builder, "remove_studs"); // priority 1
	...
	public void step_grid() {
		if (!inQueue) { // WENN NICHT IN SCHLANGE
				...
				if (current instanceof Kasse) { // Wenn du an einer Kasse stehst
					...
					if (chooseMeal(this.current)) {
						...
						if (additionallywants != -1) {
							Ausgabe tmp = null;
							if (additionallywants == consts.ESSEN_POMMES)
								tmp = sharedstuff.pommesbar;
							if (additionallywants == consts.ESSEN_SALAD)
								tmp = sharedstuff.salatbar;
							if (tmp.getStudentsInQueue() < consts.SIZE_Y - 5)
								current = tmp;
							else {
								// throw new IndexOutOfBoundsException(this+": Salat/Pommesbar zu voll! Weiß
			...
			int[] nextPos = this.current.moveForwardInQueue(this); // Sieh nach ob du eine Position weiter aufruecken
																	// kannst
	...
	public void getInQueue() {
		...
		inQueue = true;
		// wenn du in der queue bist sollst du nicht mehr eigenständiges movement machen
		// sondern kriegst von der schlange immer aufs neue gesagt dass du dich 1x
		// bewegen darst.
		// warum? damit keine lücken in der schlange entstehen, per prioritäten bewegt
		// sich der erste zuerst, ...
		sharedstuff.remove_these.add(this);
		sharedstuff.schedule.schedule(ScheduleParameters.createOneTime(sharedstuff.schedule.getTickCount() + 0.01, 1),
				sharedstuff.builder, "remove_studs"); // priority 1
	}
	...
	public void update() {
		...
		if (this.waiting)
			return; // return wenn du warten musst
		if (this.current == null)
			return; // return wenn du an erster Stelle standest und erst im naechsten Schritt eine
					// neue Loc bekommst
	...
	public Kasse toKasse() {
		...
		Kasse randomCheckout = sharedstuff.kassen
				.get(allowed_indices.get(RandomHelper.nextIntFromTo(0, allowed_indices.size() - 1)));
	...
	public List<Ausgabe> setConsideredBars(int wants_salad_or_fries) {
		...
		if (this.food_preference == consts.MEAT)
			temp = consts.meatlover;
		else if (this.food_preference == consts.VEGGIE)
			temp = consts.vegetarian;
		else if (this.food_preference == consts.VEGANER)
			temp = consts.vegan;
		else
			temp = consts.noPref;
		for (Ausgabe bar : sharedstuff.ausgaben) {
			if (temp.contains(bar.essen))
				if (bar.essen != wants_salad_or_fries) // wenn er ZUSÄTZLICH salad or fries will will er's nicht als
														// Hauptmahlzeit
					res.add(bar);
		}
		return res;
	...
	public double calcMeanSpentTicks() {
		...
		if (this instanceof StudentShortestQueue)
			temp = sharedstuff.studierendeQueue;
		else if (this instanceof StudentChaotic)
			temp = sharedstuff.studierendeChaotic;
		else if (this instanceof StudentGoalOriented)
			temp = sharedstuff.studierendeGoal;
		else
			temp = sharedstuff.studierende;
		int sum = 0;
		for (Student s : temp) {
...
//		return spentTicks;
	...
	public Student(int num, SharedStuff sharedstuff, int fp, Context<Object> context, ContinuousSpace<Object> s) {
		...
		scheduledSteps = new ArrayList<ISchedulableAction>();
		scheduledSteps.add(sharedstuff.schedule.schedule(
				ScheduleParameters.createRepeating(sharedstuff.schedule.getTickCount() + 1, 1), this, "step"));
	}
	...
	public Vector2d walk_but_dont_bump(Object to_obj) {
		...
		List<Integer> between = sharedstuff.mgrid.Bresenham((int) mypos.getX(), (int) mypos.getY(),
				(int) thatpos.getX(), (int) thatpos.getY());
...
//				}
				keepZwischenziel_stoodfor++; // das hier weg, dafür das oben hin, und hier drunter stattdessen more like
												// > 50
				if (keepZwischenziel_stoodfor > 200000) { // TODO statt das ne gewisse Zeit zu machen soll der gucken
															// ob's erfolgreich ist
					keepZwischenziel = new Vector2d(0, 0);
					keepZwischenziel_mypos = new Vector2d(0, 0);
					keepZwischenziel_stoodfor = 0;
				}
			} else {
				if (Math.abs((int) mypos.getX() - (int) thatpos.getX()) > Math
						.abs((int) mypos.getY() - (int) thatpos.getY())) {
					// wenn also die x-differenz relevanter ist als die y-differenz -> mache
					// schlenker in y-diff.
					// hard-coden dass wenn sie vor der aktionstheke stehen nicht gegen die Wand
					// laufen sollen
					...
					// wenn also die y-differenz relevanter ist als die x-differenz -> mache
					// schlenker in x-diff
	...
	public Ausgabe at_bar() {
		...
		ContinuousWithin ausgabeInRange = new ContinuousWithin(space, this, 3); // TODO das soll 'ne funktion von der
																				// Ausgabe sein, dann wird sie seltener
																				// ausgeführt!! (die ausgabe kann ja ne
																				// liste führen wer an ihr ist, und der
																				// student muss das nur abfragen)
	...
	public void step() {
		...
		// Priorität 2) Laufe zu Ausgaben (returns null wenn er gerade was zu essen
		// gefunden hat, nicht mehr hungrig ist, oder schon alle Theken besucht hat)
	...
	public void do_move() {
		...
		// System.out.println("Student #"+num+" did something"+pos.getX()+"
		// "+pos.getY()+" velocity "+velocity.x+" "+velocity.y);
		if (potentialcoords.x <= 0 || potentialcoords.x >= consts.SIZE_X || potentialcoords.y <= 0
				|| potentialcoords.y >= consts.SIZE_Y) {
		...
		if (potential_grid_pos == consts.GRID_STUDENT) {// studenten sind +2, also ist dann schon wer da
			// throw new java.lang.RuntimeException("Student laeuft auf anderen
			// Studenten!");
		}
		boolean triedkeepwalking = false;
		int durchlauf = 0;
		while ((potential_grid_pos > consts.GRID_STUDENT) || ((velocity.x == 0) && (velocity.y == 0))) { // theken,
																											// kassen,
																											// accesspoints
			if (triedkeepwalking) {
			...
			if (durchlauf++ > 50) {
				...
				if (velocity.x < 00.1 * walking_speed && velocity.y < 00.1 * walking_speed) {
					if (((keepWalkingdirection.x == 0) && (keepWalkingdirection.y == 0))) { // wenn er also nicht vorher
																							// schon gegen 'ne Wand
																							// gelaufen wäre
						do {
							velocity.x = RandomHelper.nextIntFromTo(-1, 1);
					...
					int bottomofthat = sharedstuff.mgrid.get((int) potentialcoords.x, (int) potentialcoords.y + 1); // TODO
																													// fehler
																													// fangen
